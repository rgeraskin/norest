...
| `layouts/_default/list.html` | Same as theme, except the "Remove filter" link on taxonomy term pages points to `/` instead of `/blog/` | Theme hard-codes `href="{{ "blog" \| relURL }}"`, which 404s here: `content/blog/_index.md` has `[build] render = "never"` (we suppressed the section index so it doesn't duplicate the homepage). Our homepage is the post archive, so "Remove filter" should land there. |
| `layouts/partials/footer.html` | Emits `{{ .Site.Copyright }}` (unless `hideCopyrightLine = true`) and the made-with line (unless `hideMadeWithLine = true`) | Theme's footer renders only the made-with line. Bearblog never surfaces `.Site.Copyright` anywhere visible — we add it, mirroring bearblog's `hideMadeWithLine` toggle pattern via `hideCopyrightLine`. |
...
| `layouts/partials/custom_head.html` | Injects custom `<style>` block: blockquote outdent override (no outer indent, 2px left bar aligned with text edge); `content img` centered via `display: block` + `margin: auto`; post body justified with `text-align: justify` + `hyphens: auto` (and `-webkit-hyphens` for Safari <17); `.title-tagline { white-space: nowrap }` keeps the bear-emoji tagline whole on narrow viewports | Theme provides this partial empty for user CSS overrides. Using it instead of forking `partials/style.html` keeps the theme submodule update-safe. `baseof.html` loads `custom_head.html` after `style.html`, so on equal-specificity selectors our rules win the cascade. `<content>` is a non-standard element that defaults to inline, so `display: block` is required for `text-align` to apply. `hyphens: auto` needs `lang` on `<html>` (set by `languageCode = "ru-RU"` in `hugo.toml`). |
| `layouts/partials/header.html` | Wraps the `for the Geeked ʕ´༎ຶᴥ༎ຶ`ʔ` portion of `.Site.Title` in a `<span class="title-tagline">` via `replaceRE` + `safeHTML` | Theme's version renders the bare `.Site.Title` inside `<h2>`. The bear emoji is a 9-character mixed-script sequence (Latin, Tibetan, Lao, ASCII) — by UAX #14 line-breaking rules, browsers see break opportunities at script boundaries and split it mid-glyph on narrow viewports (iPhone 12 ≈ 350px content width). Wrapping the tagline + emoji in a `white-space: nowrap` span forces a single break point between "No Rest" and the rest. **Regex hardcodes "for the Geeked ʕ"** — if `.Site.Title` changes, update this partial or extract the tagline into a separate `params` field. |
